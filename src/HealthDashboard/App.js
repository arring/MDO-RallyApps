(function(){
	...
	Ext.define('Intel.HealthDashboard', {
		...
		launch: function() {
			...
			me.configureIntelRallyApp()
				.then(function(){
					me.codashGrid = null;
					me.ScopedProjectName = Rally.environment.getContext().getProject().Name;
					me.TeamStates = Ext.create('Ext.data.Store', {
						fields: ['abbr', 'name'],
						data: [
					...
					me.chartCI = { selected: me.getSetting("showCI"), dirty: false };
					me.timeHorizon = me.getSetting("timeHorizon");
					me.gridSize = me.timeHorizon * 50;
					me.getchartSize();
					me.addSettingsPanels();
					me.myVals = ['none_Delete_this_Line_To_See_All_in_Scope'];
					me.queryIterations(me.myVals);
				})
		...
		config: {
			defaultSettings: {
				...
				VelocityVaration: 10,
				improvementTip: [
					'Improvement is the sprint percent of	all',
					'improvement accepted story points to total of accepted points.'
					'for more ellaborate explanation please click the header of the '
					'column which is an active link to the help page on the subject.'
				].join(' ')
			}
		},

		/**
			queryIteration retrieves all iterations in scope that ended before today and after one year ago.
		*/
		queryIterations: function (vals) {
			var me = this,
				today = new Date(),
				lastYear = new Date(today.getDate()-365),
				todayISO = Rally.util.DateTime.toIsoString(today, false),
				lastYearISO = Rally.util.DateTime.toIsoString(lastYear, false),
				configs = null;
			
			me.chartCI.dirty = false;
