    ...
    Ext.define('Intel.lib.mixin.CumulativeFlowChartMixin', {
        ...
        _getRSquared: function (ySeries, fSeries, lastIndex) {
            ...
            var ys = ySeries.data.slice(0, lastIndex),
                ...
                meanY = _.reduce(ys, function (sum, yi) {
                        return sum + (yi || 0);
                    }, 0) / ys.length,
                SStot = _.reduce(ys, function (sum, yi) {
                    return sum + Math.pow((yi || 0) - meanY, 2);
                }, 0),
                SSres = _.reduce(fs, function (sum, fi, i) {
                    return sum + Math.pow((ys[i] || 0) - (fi || 0), 2);
                }, 0);
        ...
        _addProjectedTrendline: function (data, options) {
            var me = this,
                ...
                slope, intercept, X, Y,
                scheduleStateSeries = _.filter(data.series, function (s) {
                    return me.ScheduleStates.indexOf(s.name) > -1;
                }),
                scheduleStatesSumList = _.times(scheduleStateSeries[0].length, function (n) {
                    return _.reduce(scheduleStateSeries, function (sum, s) {
                        return sum + (s.data[n] || 0);
                    }, 0);
                });
            trendType = _.find(validTypes, function (type) {
                return type == trendType;
            }) || validTypes[0];

            //initialize projected trendline
            var topScheduleState = me.ScheduleStates.slice(-1)[0],
                topScheduleStateSeries = _.find(data.series, function (s) {
                    return s.name === topScheduleState;
                }), i, len,
                projectedTrend = {
                    type: 'spline',
                    dashStyle: 'Solid',
                    name: 'Projected',
                    data: topScheduleStateSeries.data.slice()
                },
                begin = 0,
            ...
            if (trendType == 'LinearRegression') {
                ...
                X = $M(_.map(projectedTrend.data.slice(begin, end), function (p, j) {
                    return [1, j];
                }));
                Y = $M(_.map(projectedTrend.data.slice(begin, end), function (p) {
                    return p;
                }));
        ...
        _getIndexOnOrAfter: function (date, dateArray) {
            ...
            if (pos === dateArray.length - 1) {
                if (dateArray[pos] >= date) return pos; else return -1;
            } //either start of list or everything is after 'date'
        ...
        updateCumulativeFlowChartData: function (data, options) {
            ...
            var totalPoints = (new Date(data.categories[0]) > now ? 0 :
                _.reduce(data.series, function (sum, s) {
            ...
            if (!hideTrends) {
                idealTrend = {
                    ...
                    dashStyle: 'Solid',
                    name: 'Ideal',
                    data: new Array(data.categories.length)
                };
                ratio = (totalPoints / (data.categories.length - 1)) || 0; //for NaN
                idealTrend.data = _.map(idealTrend.data, function (e, i) {
                    return Math.round(100 * (0 + i * ratio)) / 100;
                });
            }
            ...
            _.each(data.categories, function (c, i, a) {
                ...
                if (d > now) {
                    _.each(data.series, function (s, j) {
                        s.data = s.data.slice(0, i).concat(_.map(new Array(a.length - i), function () {
                            return 0;
                        }));
                    });
                }
        ...
        setCumulativeFlowChartDatemap: function (chartContainerId, datemap) {
            var me = this;
            /**
             * The projected end date and work week code here is needed specifically for
             * Portfolio Navigator project.
             */
            var projectedDate = datemap[savedValue];
            var d = new Date(projectedDate);
            var projectedWorkWeek = 'WW' + me.getWorkweek(d);
            if (isNaN(me.getWorkweek(d)) || projectedDate == undefined) {
                me.projectedEndDate = "None";
            } else {
                me.projectedEndDate = projectedWorkWeek + " (" + projectedDate + ")";
            }
            //Set datemap for chart tooltips
