(function() {
	...
	Ext.define('ContinuousImprovementReport', {
		...
		launch: function() {
			...
			.then(function() {
				console.log(me.ScrumGroupConfig);
			...
			.then(function(release) {
				...
				// DEBUG commented out return me.loadProjectByName(me.Team.Train + ' POWG Portfolios');
				return me.loadProject(_.find(me.ScrumGroupConfig, function(group) {return group.ScrumGroupName.indexOf(me.Team.Train) > -1;}).PortfolioProjectOID);
			})
			...
			.then(function(productStore) {
				...
				ciProduct = _.find(productStore.getRange(), function(p) {return p.data.Name.indexOf('STDCI') > -1;});
			...
			.then(function() {
				if (me.isShowingAllReleases) return me._loadIterations();
			})
			// Load the UI
		...
		_loadStoriesUnderFeatures: function(featureStores) {
			...
			for (var i in featureStores) {
				...
				me.Features = me.Features.concat(features);
		...
		_releaseCheckboxChanged: function(box, newVal, oldVal, opts) {
			...
			return me._updatePreferences().then(function() {
		...
		/**************************************** Preferences *************************************/
		/*
		 *	Loads the preferences for this app
		 */
		_loadPreferences: function() {
			...
			Rally.data.PreferenceManager.load({
				...
				success: function(preferences) {
					if (preferences.showAllReleases) {
						me.isShowingAllReleases = preferences.showAllReleases;
					}
					else {
		...
		/*
		 *	Saves the preferences for this app
		 */
		_updatePreferences: function() {
			...
			Rally.data.PreferenceManager.update({
				...
				workspace: me.getContext().getWorkspace()._ref,
				settings: {
					showAllReleases: me.isShowingAllReleases
				},
				success: function() {
					deferred.resolve();
				}
