    ...
    Ext.define('Intel.SAFe.ArtCommitMatrix', {
        ...
        getTotalUserStoryPoints: function (userStoriesData) {
            return _.reduce(userStoriesData, function (sum, userStoryData) {
                return sum + (userStoryData.PlanEstimate || 0);
            }, 0);
        },
        ...
        getCellCls: function (config) {
            ...
            if (me.ViewMode == 'Normal') {
                switch (config.commitment) {
                    case 'Undecided':
                        cls = colorClassBase + 'WHITE';
                        break;
                    case 'N/A':
                        cls = colorClassBase + 'GREY';
                        break;
                    case 'Committed':
                        cls = colorClassBase + 'GREEN';
                        break;
                    case 'Not Committed':
                        cls = colorClassBase + 'RED';
                        break;
                    default:
                        cls = colorClassBase + 'WHITE';
                        break;
        ...
        areColorsTheSame: function (color1, color2) {
            ...
            if (_.some(nums1, function (num1, index) {
                    return Math.abs(num1 - nums2[index]) * 100 >> 0 > 0;
                })) return false;
            return true;
        },
        updateCell: function (portfolioItemRecord, projectName, rowIndex, columnIndex) {
            ...
            var tableRowDOM = me.MatrixGrid.view.getNode(rowIndex),
                ...
                oldColorClass = _.find(classList, function (c) {
                        return c.indexOf(colorClassBase) > -1;
                    }) || '';
            ...
            if (((config.expected && config.featurestatus && !config.ceComment) && !(oldExpected && !oldComment && !oldExpectedComment)) ||
                ...
                _.each(newCls.split(' '), function (cls) {
                    if (cls.length) td.classList.add(cls);
                });
                //innerHTML
        ...
        updateGridHeader: function (projectName) {
            ...
            var column = _.find(me.MatrixGrid.view.getGridColumns(), function (column) {
                    return column.text == projectName;
                }),
                possibleClasses = ['not-dispositioned-project', 'dispositioned-project'],
                shouldHaveItems = me.ViewMode === '% Done';
            if (column) {
                _.each(possibleClasses, function (cls) {
                    column.el.removeCls(cls);
                });
        ...
        reloadEverything: function () {
            ...
            me.enqueue(function (done) {
                return me.reloadStores()
                    ...
                    .then(function () {
                        if (me.IsDataRefresh === false) me.updateCache();
                    })
                    .fail(function (reason) {
        ...
        setCachePayLoadFn: function (payload) {
            ...
            payload.AllProjects = _.map(me.AllProjects, function (ap) {
                return filterProjectData(ap.data);
            });
            payload.ReleaseRecords = _.map(me.ReleaseRecords, function (rr) {
                return {data: rr.data};
            });
            payload.ReleaseRecord = {data: me.ReleaseRecord.data};
            payload.PortfolioItemTypes = me.PortfolioItemTypes;
            payload.PortfolioItemStoreData = _.map(me.PortfolioItemStore.getRange(), function (ps) {
                return filterPortfolioItemForCache(ps.data);
            });
                                    ...
                                    return me.loadScrumGroupPortfolioProject(me.ScrumGroupRootRecord)
                                        .then(function (scrumGroupPortfolioProject) {
                                            ...
                                            me.ScrumGroupAndPortfolioConfig = _.filter(me.ScrumGroupConfig, function (train) {
                                                return train.ScrumGroupRootProjectOID === me.ProjectRecord.data.ObjectID;
                                            })[0];
                                        });
                                }
        ...
        clearFiltersButtonClicked: function () {
            ...
            if (me.MatrixGrid) {
                ...
                _.invoke(Ext.ComponentQuery.query('intelgriduncommittedfeaturefilter', me.MatrixGrid), 'clearFilters');
        ...
        renderMatrixGrid: function () {
            ...
            /* MoSCoWRanks = ['Must Have', 'Should Have', 'Could Have', 'Won\'t Have', 'Undefined', ''], */
                sortedPortfolioItems = _.sortBy(me.PortfolioItemStore.getRange(), function (p) {
                    return p.data.DragAndDropRank;
                }),
            ...
            var matrixStore = Ext.create('Intel.lib.component.Store', {
                ...
                intelUpdate: function () {
                    ...
                    _.each(projectNames, function (projectName) {
                        me.updateGridHeader(projectName);
                    });
            ...
            var lockedColumns = [/* {
             ...
             sortFn: function(MoSCoW){ return MoSCoWRanks.indexOf(MoSCoW); }
                ...
                renderer: function (formattedID, meta, matrixRecord) {
                    var portfolioItemRecord = _.find(me.PortfolioItemStore.getRange(), function (item) {
                        return item.data.FormattedID == formattedID;
                    });
                    var totalFeatureHelp = _.filter(me.getTeamCommits(portfolioItemRecord), function (val, key) {
                        return val.FeatureStatus === true;
                    });
                ...
                renderer: function (obejctID, metaData, matrixRecord, row, col) {
                    ...
                    var portfolioItemRecord = _.find(me.PortfolioItemStore.getRange(), function (item) {
                        return item.data.ObjectID == obejctID;
                    });
                    if (!portfolioItemRecord) return;
                    var config = _.reduce(_.sortBy(_.keys(me.MatrixUserStoryBreakdown)), function (sumConfig, projectName) {
                        var teamCommit = me.getTeamCommit(portfolioItemRecord, projectName),
            ...
            if (me.HorizontalGroupingConfig.enabled) {
                ...
                teamColumnCfgs = _.map(_.groupBy(_.sortBy(_.map(teamColumnCfgs,
                                function (teamColumnCfg) {
                                    return {
                                        teamTypeInfo: _.find(allTeamTypeInfos, function (tti) {
                                            return tti.projectRecord.data.Name === teamColumnCfg.text;
                                        }),
                        ...
                        function (item) {
                            return item.teamTypeInfo ? item.teamTypeInfo.horizontal : 'null';
                        }),
            ...
            var columns = _.map(lockedColumns.concat(teamColumnCfgs), function (colDef) {
                return _.merge({}, COLUMN_DEFAULTS, colDef);
            });
            ...
            setTimeout(function () {
                _.each(_.keys(me.MatrixUserStoryBreakdown), function (projectName) {
                    me.updateGridHeader(projectName);
                });
            }, 50);
        }
