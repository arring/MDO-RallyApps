	...
	Ext.define('Intel.SAFe.TeamReport', {
  	...
  	getDefectFilter: function(){
		...
		reloadStores: function(){
			...
			if(!me.isEditingVelocity) {
                 promises.push(me.loadIterations());
          
            }
            
			if(!me.isEditingTeamCommits) promises.push(me.loadPortfolioItems());
			if(!me.isEditingVelocity && !me.isEditingTeamCommits && !isEditingDeps) {
                promises.push(me.loadUserStories());
                promises.push(me.loadDefects());
            }
		...
		renderVelocityGrid: function() {
			...
			var iterationGroupTotals = _.map(_.sortBy(me.IterationStore.getRecords(), 
				...
				function(iteration) {
                    ...
                    var defectVelocity = iteration.data.PlannedVelocity;
                              var totalDefectPlanEstimate=0;                            
                            _.each(me.DefectsStore.getRange(), function(defectRecord){
                        
                              if(defectRecord.data.Iteration !== null)
                              if( defectRecord.data.Iteration._refObjectName===iName)   
                                    totalDefectPlanEstimate= totalDefectPlanEstimate+ defectRecord.data.PlanEstimate;
                           
                    });  
					return {    
						Name: iName, 
						PlannedVelocity: iteration.data.PlannedVelocity || 0,
						RealVelocity: totalDefectPlanEstimate+ _.reduce((iterationGroups[iName] || []), function(sum, us) {                                                        
			...
			var velocityColumns = [{	
				...
				renderer:function(realVel, meta, record){
                    ...
                    var perct = record.data.PlannedVelocity >0? (record.data.RealVelocity/record.data.PlannedVelocity)*100:0;                   
                 return '<span title="' + Math.round(perct) + ' %">' + realVel + '</span>';
				}
			}];		
				...
				renderer:function(value, meta){
					...
					var planned = _.reduce(me.IterationStore.getRecords(), function(sum, i){ return sum + (i.data.PlannedVelocity || 0); }, 0),
						real = _.reduce(me.IterationStore.getRecords(), function(bigSum, iteration){
                              totalDefectPlanEstimate=0;                  
                            _.each(me.DefectsStore.getRange(), function(defectRecord){                                                        
                              if(defectRecord.data.Iteration !== null)
                              if( defectRecord.data.Iteration._refObjectName===iteration.data.Name)   
                                    totalDefectPlanEstimate= totalDefectPlanEstimate + defectRecord.data.PlanEstimate;
                           
                            });                                                         
							return  totalDefectPlanEstimate+bigSum + _.reduce((iterationGroups[iteration.data.Name] || []), function(sum, us) {
								return sum + 
                    ...
                    var perct=real > 0? (real/planned)*100:0;
                      return '<span title="' + Math.round(perct) + ' %">' + real + '</span>';
				}
			}];
