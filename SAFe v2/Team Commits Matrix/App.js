	...
	Ext.define('CommitMatrix', {
		...
		_loadFeatures: function(){
			...
			// return _.reduce(piDataObject, function(lowestPiOIDsList, nextPiData, nextPiOID){
				...
				// if(Object.keys(nextPiData).length ===0) return lowestPiOIDsList.concat([nextPiOID]);
				// else return lowestPiOIDsList.concat(me._getLowestPiItems(nextPiData));
			// }, []);
		// },
		// _loadPortfolioItemMapping: function(){
			// var me = this,
				// topPiTypeName = me.PortfolioItemTypes[me.PortfolioItemTypes.length-1],
				// parallelLoaderConfig = {
					// pagesize:200,
					// model:me.PortfolioItem,
					// url: 'https://rally1.rallydev.com/slm/webservice/v2.0/PortfolioItem',
					// params: {
						// project: me.TrainPortfolioProject.data._ref,
						// projectScopeUp: false,
						// projectScopeDown: true,
						// pagesize:200,
						// fetch:'Name,Parent,ObjectID,PortfolioItemTypeName'
					// }
				// };
			...
			// return me._parallelLoadWsapiStore(parallelLoaderConfig)
				// .then(function(store){ 
					// var tree = {}, topPiOIDs = [];
					// _.each(store.getRange(), function(item){
						// if(!tree[item.data.ObjectID]) tree[item.data.ObjectID] = {};
						// tree[item.data.ObjectID].Name = item.data.Name;
						// if(item.data.Parent){
							// if(!tree[item.data.Parent.ObjectID]) tree[item.data.Parent.ObjectID] = {};
							// tree[item.data.Parent.ObjectID][item.data.ObjectID] = tree[item.data.ObjectID];
						// }
						// if(item.data.PortfolioItemTypeName == topPiTypeName) topPiOIDs.push(item.data.ObjectID);
					// });
					// _.each(topPiOIDs, function(topPiOID){
						// var lowestPiOIDs = me._getLowestPiItems(tree[topPiOID]),
							// topPiName = tree[topPiOID].Name;
						// me.TopPortfolioItemNames.push({Name:topPiName});
						// _.each(lowestPiOIDs, function(obj, lowestPiOID){ me.PortfolioItemMap[lowestPiOID] = topPiName; });
					// });
					// me.TopPortfolioItemNames = _.sortBy(me.TopPortfolioItemNames, function(topPiName){ return topPiName.Name; });
					// return store;
				// });
		// },
		/****************************************************** PROJECTS ************************************/		
					...
					for(var i=0, len=records.length; i<len; ++i){
						...
						for(var j=0;j<pnamesLength; ++j){
							var projectName = projectNames[j],
								...
								managerComment = !!(teamCommit.CEComment || false),
			...
			var defColumnCfgs = [
				{
					...
					menuDisabled:true,
					locked:true,
					renderer:function(FID, meta, matrixRecord){
						var feature = me.FeatureStore.findExactRecord('FormattedID', FID);
						var tc = me._getTeamCommitFromFeature(feature);
						if(_.some(tc, function(projData, projectOID){ return projData.Commitment == 'Not Committed'; })){
							meta.tdCls += ' red-background';
						}
