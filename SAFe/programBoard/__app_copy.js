...
Ext.define('CustomApp', {
	...
	_loadModels: function(cb){
		...
		Rally.data.ModelFactory.getModel({ //load feature
	...
	_loadFeature: function(foid, cb){ 
		var me = this;
		me.Feature.load(foid, {
			fetch: ['Name', 'ObjectID', 'FormattedID', 'c_TeamCommits', 'c_Risks'],
			context: {
				workspace: me.getContext().getWorkspace()._ref,
				project: null
			},
	...
	_buildDependenciesData: function(){	
		...
		me.DependenciesReleaseUserStories = _.filter(me.DependenciesUserStoryStore.getRecords(), function(usr){
			usr.isInRelease = usr.get('Release') && usr.get('Release').Name === me.ReleaseRecord.get('Name');
			return usr.isInRelease;
		});	
		...
		_.each(me.DependenciesUserStoryStore.getRecords(), function(userStoryRecord){ //load risks into custom Data Store
			...
			if(userStoryRecord.isInRelease){
			...
			for(var i=0; i<succs.length;++i){
				...
				if(new Date(succDep.REL) >= startDate && new Date(succDep.REL_S) <= endDate){
					...
					succDepsList.push({
						...
						_realFormattedID: userStoryRecord.get('FormattedID'), //only used by succDepGrid
						_realUserStoryName: userStoryRecord.get('Name'),			//only used by succDepGrid
						Edited: false //not in pending edit mode
					});
				}
			}
	...
	_reloadRisksStores: function(){
		var me = this, editing = me._isEditing(me.CustomRisksStore);
		if(editing) return;			
		if(me.RisksFeatureStore) {
	...
	_reloadDependenciesStores: function(){
		var me = this, 
			editing = me._isEditing(me.CustomPredDepStore) || me._isEditing(me.CustomSuccDepStore);
		if(editing) return;
		if(me.DependenciesUserStoryStore) {
			me.DependenciesUserStoryStore.load({ 
				callback: function(records, operation){
					me._buildDependenciesData(); //reparse the data
					if(me.CustomPredDepStore){
						var scroll1 = me.PredDepGrid.view.getEl().getScrollTop();
						me.CustomPredDepStore.update();
						me.PredDepGrid.view.getEl().setScrollTop(scroll1);
					}
					if(me.CustomSuccDepStore){
						var scroll2 = me.SuccDepGrid.view.getEl().getScrollTop();
						me.CustomSuccDepStore.update();
						me.SuccDepGrid.view.getEl().setScrollTop(scroll2); 
					}
				}
			});
		}
			...
			me._loadProject(scopeProject, function(scopeProjectRecord){
				me._loadRootProject(scopeProjectRecord, function(){
					me._loadValidProjects(function(){
						me.ProjectRecord = me.ValidProjects[scopeProjectRecord.data.ObjectID];
						if(me.ProjectRecord){
	...
	_loadTeamCommitsGrid: function(){
		...
		function getTeamCommit(featureRecord){	
			...
			var projectID = me.ProjectRecord.get('ObjectID');
			var this_tc;
			try{ this_tc = JSON.parse(atob(tcs))[projectID] || {}; } 
			catch(e){ this_tc = {}; }
			return this_tc;
		}
		...
		me.CustomTeamCommitsStore = Ext.create('Ext.data.Store', {
			...
			update: function(){
				var customTeamCommitsStore = me.CustomTeamCommitsStore, 
					currentTeamCommitsRecords = customTeamCommitsStore.getRange();
    ...
    var iterationGroupTotals = _.sortBy(_.map(me.VelocityIterationStore.getRecords(), function(iteration) {
		...
		me.CustomVelocityStore = Ext.create('Ext.data.Store', {
			...
			update: function(){
				var customVelocityStore = me.CustomVelocityStore, 
					velocityRecords = customVelocityStore.getRange();
	...
	_loadRisksGrid: function(){
		...
		me.CustomRisksStore = Ext.create('Ext.data.Store', { 
			...
			sorters: [riskSorter],
			update: function(){
				var customRisksStore = me.CustomRisksStore, 
					currentRisksRecords = customRisksStore.getRange(),
					...
					return {
						...
						handler: function(){
							me._confirm('Confirm', 'Delete Risk?', function(msg){
								...
								me.RisksGrid.setLoading(true);
								me._loadFeature(me.RisksFeatureStore.
									callback: function(records, operation){
										me._parseRisksData();
										var riskRecordData = riskRecord.data;
										var realRiskData = removeRiskFromList(riskRecordData.RiskID, me.RisksParsedData.slice(0));
	...
	_loadDependenciesGrids: function(){
		...
		function syncCollection(userStoryRecord, usAddList, usRemoveList, type, callback){ //type == Predecessors || Successors
			var collectionStore, collectionRecords, finished=-1, syncCollectionProxy = false;
		...
		function removePredDep(userStoryRecord, predDepData, cb){
			...
			if(userStoryRecord.get('Project').ObjectID === me.ProjectRecord.get('ObjectID'))
				_.each(cachePreds, function(cp, i){ if(cp.DependencyID===depID) cachePreds.splice(i, 1); });
		...
		me.CustomPredDepStore = Ext.create('Ext.data.Store', { 
			...
			limit:Infinity,
			update: function(){ 
				var customPredDepStore = me.CustomPredDepStore, 
					customPredDepRecs = customPredDepStore.getRange(),
				...
				for(var i = 0;i<customPredDepRecs.length;++i){
					...
					if(!preds.length) depRec.set('Predecessors', [newTeamDep()]); 
					...
					if(!me.PredDepTeamStores[depID]){
						me.PredDepTeamStores[depID] = Ext.create('Ext.data.Store', { 
							...
							update: function(){
								...
								if(depTeamStore.getRange().length===0) depTeamStore.add(newTeamDep());
							}
						});	
					}
				...
				renderer: function(value, meta, predDepRecord){	
					...
					if(dirtyType !== 'Edited') return ''; //don't render it!
						...
						handler: function(){
							...
							var scroll = me.PredDepGrid.view.getEl().getScrollTop();
							for(var key in realDep){
								if(key === 'Predecessors') predDepRecord.set(key, Ext.clone(realDep[key]) || [newTeamDep()]);
						...
						handler: function(){
							...
							me.DependenciesUserStoryStore.load({
								callback: function(userStoryRecords, operation){
									...
									var realDepData = Ext.clone(removeDepFromList(predDepData.DependencyID, realPredDeps) || {});
      ...
      title: "Dependencies We Have on Other Teams",
		...
		me.CustomSuccDepStore = Ext.create('Ext.data.Store', { 
			...
			sorters:[depSorter],
			update: function(){
				var customSuccDepStore = me.CustomSuccDepStore,
					customSuccDepRecs = customSuccDepStore.getRange(), 
